...
public class IsNotNullTranspiler implements SExpressionTranspiler {
  ...
  public Try<Expression<?>> apply(SExpression<?> sExpr, TranspileContext context) {
    ...
    return field.map(f -> Expressions.predicate(Ops.IS_NOT_NULL, f));
  }


  @Override
  public Boolean support(SExpression<?> sExpr) {
    return Operators.IS_NOT_NULL == sExpr.getOperator();
  }
}
